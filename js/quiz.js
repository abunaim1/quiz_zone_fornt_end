...
// const displayQuestionPaperByType = (div, item) => {
//   const question_paper = document.getElementById("question_paper");
...
//   `
//   <div class="card-body" >
//   <h5 class="card-title text-center" style="font-size:17px;"><a type="button" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="quesionPopUp('${item.name}', '${item.difficulty}', ${item.id}, ${item.mark}, '${item.question_category}', '${item.time}')">${item.name}</a></h5>
//   </div>
...
const displayQuestionPaper = (data) => {
  ...
  data[0].question_paper.forEach((item) => {
    ...
    div.innerHTML = `
      ...
      <div class="card-body" >
        <h5 class="card-title text-center" style="font-size:17px;"><a type="button" class="card-title text-center" data-bs-toggle="modal" data-bs-target="#exampleModal" onclick="quesionPopUp('${item.name}', '${item.difficulty}', ${item.id}, ${item.mark}, '${item.question_category}', '${item.time}')">${item.name}</a></h5>
      </div>
      </div>
       `;
    question_paper.appendChild(div);
...
const quesionPopUp = (name, difficulty, id, mark, question_category, time) => {
  ...
  div2.innerHTML = `
    ...
    <a href="question.html?question_category=${name}" target="_blank" type="button" class="button">Start Quiz</a>
    `;
